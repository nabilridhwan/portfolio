...
export default function CurrentlyPlaying({ setError, setType }: MusicPlayerProps) {
	...
	useEffect(() => {
		if (status === 'success') {
			...
			const artist_string = data.item?.artists.map((a: any) => a.name).join(', ');
	...
	return (
		<Link href={spotifyLink}>
			<div className="flex items-center gap-3 cursor-pointer">
				...
				{imageUrl && (
					<Image src={imageUrl} alt={songName} width={30} height={30} className="aspect-square rounded-md border border-white" />
				)}

				<div className={'flex-col flex gap-0'}>
					<p className="italic font-bold text-sm line-clamp-1">{songName}</p>
