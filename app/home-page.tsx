...
export default function HomePage({ skills, testimonials }: HomePageProps) {
	...
	return (
		<Container>
			<motion.div id="hero-section" className="relative mt-[40px] bg-primary rounded-[20px] grid md:grid-cols-2 md:grid-rows-1 grid-rows-0">
					...
					<section className="my-5">
						<Link href="mailto:nabridhwan+p@gmail.com">
							<span className="flex flex-row items-center gap-2 cursor-pointer">
								...
								<p className="font-bold">Have an idea worth building? Let’s bring it to life.</p>
							</span>
						</Link>
					</section>
				...
				<section className="relative -z-[0]">
					...
					>
						<Image src="/hero-image.png" alt="Portrait of Nabil Ridhwan" width={420} height={420} priority />
					</motion.div>
				</section>
			...
			<section className="relative overflow-clip">
				...
				<motion.div style={{ x: xTranslation }} className="py-6 flex gap-[20px] items-center" ref={ref}>
					{[...skills, ...skills].map((skill, index) => (
						<span key={`${skill}-${index}`} className="font-bold whitespace-nowrap">
							{skill.toUpperCase()}
						</span>
					))}
				</motion.div>
